...
class StreamCompiler:
  ...
  def _handle_init_stream_surface(self, event: InitStreamSurfaceEvent) -> list[A2UIFrame]:
    ...
    return self._compile(
        InitSurfaceDelta(
            ...
            summary='固定竖排增量页面（streaming block-first）',
        )
    )
  ...
  def _handle_create_text_block(self, event: CreateTextBlockEvent) -> list[A2UIFrame]:
    ...
    frames.extend(self._emit_text_lines(event.block_id, event.lines))
    return frames

  def _handle_append_text_lines(self, event: AppendTextLinesEvent) -> list[A2UIFrame]:
    self._ensure_surface_initialized()
    self._require_block(event.block_id, expected_type='text')
    return self._emit_text_lines(event.block_id, event.lines)
  ...
  def _handle_create_list_block(self, event: CreateListBlockEvent) -> list[A2UIFrame]:
    ...
    frames.extend(self._emit_block_title(event.block_id, event.title))
    frames.extend(self._emit_list_items(event.block_id, event.items))
    return frames

  def _handle_append_list_items(self, event: AppendListItemsEvent) -> list[A2UIFrame]:
    self._ensure_surface_initialized()
    self._require_block(event.block_id, expected_type='list')
    return self._emit_list_items(event.block_id, event.items)
  ...
  def _handle_append_table_rows(self, event: AppendTableRowsEvent) -> list[A2UIFrame]:
    ...
    # 第一版折中：追加 rows 后重建完整 spec_json，再次提交 AddTableDelta。
    # 这样可复用现有 FrameCompiler，不必新增 append_table_rows 的 low-level 协议。
    cache.rows.extend(event.rows)
    return self._emit_table_component(
        block_id=event.block_id,
        table_component_id=cache.table_component_id,
        title=cache.title,
